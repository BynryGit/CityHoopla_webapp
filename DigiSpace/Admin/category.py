...
def cheksamesequence(zipped_list):
	for city_id,sequence in zipped_list:
		if city_id!='' and sequence!='':
			...
			try:
				cat_obj = CategoryCityMap.objects.get(city_place_id=City_Place.objects.get(city_place_id=city_id),sequence=sequence)
			except:
				return True	
...
def updatecheksamesequence(zipped_list,cat_id):
	...
	for city_id,sequence in zipped_list:
		if city_id!='' and sequence!='':
			cat_obj = CategoryCityMap.objects.filter(city_place_id=City_Place.objects.get(city_place_id=city_id),sequence=sequence).exclude(category_id=cat_obj.category_id)
			if cat_obj:
				return False
			else:
				return True	
		...
		if city_list!=[u'']:	
			...
			if(updatecheksamesequence(zipped_list,category_id)):
				...
				if(str(category_id)==str(category_object)):
					...
					subcategory_list1 = request.POST.getlist('subcategory1[]')
					if subcategory_list1!=[u'']:
						save_subcategory(1,subcategory_list1,category_object)


					subcategory_list2 = request.POST.getlist('subcategory2[]')
					if subcategory_list2!=[u'']:
						save_subcategory(2,subcategory_list2,category_object)


					subcategory_list3 = request.POST.getlist('subcategory3[]')
					if subcategory_list3!=[u'']:
						save_subcategory(3,subcategory_list3,category_object)

					subcategory_list4 = request.POST.getlist('subcategory4[]')
					if subcategory_list4!=[u'']:
						save_subcategory(4,subcategory_list4,category_object)

					subcategory_list5 = request.POST.getlist('subcategory5[]')
					if subcategory_list5!=[u'']:
						save_subcategory(5,subcategory_list5,category_object)
